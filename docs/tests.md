...
| TC-HA-13 | FR-IE-05, FR-IE-06 | TestPostInfer | Success path returns output and latency | Verify a successful request returns generated text plus timing and per-node memory fields. | ModelRegistry with mamba-130m registered; `plan_dispatch`, `ModelStore`, `PipelineRunner`, `AutoTokenizer.from_pretrained` mocked; one-node fake plan. | max_new_tokens=1. | 1. Call `POST /infer`. | 200; output="hello world"; node_latencies_ms=[5.0]; peak_memory_mb=[260]; num_nodes=1; latency_ms >= 0; runner.load called once. |
...
Component under test: `components/pages/dashboard.tsx`, wired to `listNodes()` and `getInferenceLog()`. `listNodes` is mocked via `vi.spyOn(api, 'listNodes')`; the inference log is read straight from `localStorage` (cleared after each test), matching how the real Inference page writes to it via `appendInferenceLog()`. Latencies render through the `LatencyChart` component (`components/latency-chart.tsx`), a plain SVG line chart (single hue, one series needs no legend, end-point and hover markers) sized via `viewBox` + percentage width/height so it fills whatever vertical space the page gives it rather than rendering at a small fixed size.
