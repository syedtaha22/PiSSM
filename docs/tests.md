...
| TC-AC-02 | FR-WUI-03 | listModels | Calls GET /models | Verify `listModels()` calls `fetch` with the correct path and returns the parsed body. | `fetch` mocked to resolve 200 with a model array. | `[{ name: 'mamba-130m' }]`. | 1. Call `listModels()`. | `fetch` called with `/models`; returns the mocked array. |
| TC-AC-03 | FR-WUI-03 | submitModel | POSTs manifest YAML | Verify `submitModel()` POSTs the YAML wrapped as `{manifest_yaml}` and returns the registered model. | `fetch` mocked to resolve 201. | `manifestYaml="name: mamba-130m\n"`. | 1. Call `submitModel(manifestYaml)`. | `fetch` called with `POST /models`, body `{"manifest_yaml": ...}`; returns the mocked model. |
...
| TC-AC-06 | FR-WUI-04 | runInference | POSTs model_name and input | Verify `runInference()` POSTs the expected body and returns the parsed `InferResult`. | `fetch` mocked to resolve 200 with a full inference result. | `modelName="mamba-130m"`, `input="hi"`. | 1. Call `runInference(modelName, input)`. | `fetch` called with `POST /infer`, correct body; result has `output`/`num_nodes`. |
| TC-AC-07 | FR-WUI-04 | runInference | Includes max_new_tokens when provided | Verify the optional `maxNewTokens` argument is included in the request body only when given. | `fetch` mocked to resolve 200. | `maxNewTokens=5`. | 1. Call `runInference('mamba-130m', 'hi', 5)`. | `fetch` body includes `"max_new_tokens": 5`. |
...
| TC-AC-09 | FR-WUI-04 | loadModel | POSTs to /models/{name}/load | Verify `loadModel()` POSTs to the correct path and returns the parsed status. | `fetch` mocked to resolve 200 with a loading status. | `modelName="mamba-130m"`. | 1. Call `loadModel(modelName)`. | `fetch` called with `POST /models/mamba-130m/load`; returns `{status: 'loading', ...}`. |
...
Component under test: `components/pages/dashboard.tsx`, wired to `listNodes()` and `getInferenceLog()`. `listNodes` is mocked via `vi.spyOn(api, 'listNodes')`; the inference log is read straight from `localStorage` (cleared after each test), matching how the real Inference page writes to it via `appendInferenceLog()`. Latencies render through the `LatencyChart` component (`components/latency-chart.tsx`), a plain SVG line chart (single hue, one series needs no legend, end-point and hover markers, no area fill) with gridlines at 0/25/50/75/100% of the max latency, sized via `viewBox` + percentage width/height so it fills whatever vertical space the page gives it rather than rendering at a small fixed size.
...
| TC-DB-04 | FR-WUI-02, FR-WUI-06 | Dashboard | Plots latencies from the inference history log | Verify the latency chart section reflects the real logged entry count. | `localStorage` pre-seeded with 2 entries. | latencyMs=120 and 340. | 1. Render `<Dashboard />`. | "Inference Latency (2 prompts)" heading present in the document. |
...
| Test Case ID | Requirement | Test Suite | Title | Description | Pre-conditions | Test Data | Test Steps | Expected Result |
|---|---|---|---|---|---|---|---|---|
| TC-MD-01 | FR-WUI-03 | Models | Renders the model list from listModels() | Verify the models page renders a real manifest returned by `listModels()`. | `listModels` mocked to resolve one manifest. | Full mamba-130m manifest. | 1. Render `<Models />`. 2. Wait for async update. | "Models" heading and "mamba-130m" entry present in the document. |
...
Component under test: `components/pages/inference.tsx`, wired to `listModels()`, `loadModel()`, `redistributeModel()`, `getModelStatus()`, and `runInference()`. All five are mocked via `vi.spyOn`. Selecting a model triggers an immediate `loadModel()` call followed by polling `getModelStatus()`; the prompt input and send button stay disabled until the model reports "ready". The Redistribute button re-dispatches the current model across whatever nodes are available now, via the same load-and-poll flow but calling `redistributeModel()` instead. Message bubbles wrap long text (`break-words whitespace-pre-wrap`, `overflow-x-hidden` on the transcript) instead of overflowing horizontally. The prompt input regains focus once a request completes (it's disabled mid-request, which drops focus), and every completed request is appended to the `localStorage`-backed inference log the Dashboard's latency chart reads.

| Test Case ID | Requirement | Test Suite | Title | Description | Pre-conditions | Test Data | Test Steps | Expected Result |
|---|---|---|---|---|---|---|---|---|
| TC-INF-01 | FR-WUI-04 | Inference | Shows a loading message while the model preloads, disabling the input | Verify selecting a model immediately starts preloading it, and the prompt input stays disabled with a progress message until it is ready. | `listModels` resolves one manifest; `loadModel`/`getModelStatus` both resolve status="loading". | Full mamba-130m manifest, num_nodes=1. | 1. Render `<Inference />`. 2. Wait for the model to appear in the dropdown. | "Loading model onto 1 node..." message shown; prompt input disabled with placeholder "Waiting for model to load...". |
| TC-INF-02 | FR-WUI-04, FR-WUI-06 | Inference | Enables the send button once ready, sends the prompt, refocuses the input, logs latency, and shows tokens/sec | Verify the prompt input enables once the model is ready; sending a prompt calls `runInference` and renders the response; the input regains focus once sending finishes; the completed request is appended to the inference history log; and the tokens/sec metric tile reflects `num_tokens / (latency_ms / 1000)`. | `loadModel`/`getModelStatus` resolve status="ready"; `runInference` resolves a full result. | prompt="hi there", output="hello world", latency_ms=120, num_tokens=20. | 1. Render `<Inference />`. 2. Wait for input to enable. 3. Type prompt. 4. Click send button (queried by accessible name "Send"). 5. Wait for the input to re-enable. | `runInference` called with `('mamba-130m', 'hi there')`; "hello world" rendered in the transcript; input is enabled and has focus; `getInferenceLog()` returns one entry matching the request; "166.7 tok/s" present (20 tokens / 0.12s). |
| TC-INF-05 | FR-WUI-04 | Inference | Clicking Redistribute re-dispatches across the current nodes | Verify the Redistribute button calls `redistributeModel()` (not `loadModel()` again) once the model is ready. | `loadModel`/`getModelStatus` resolve status="ready"; `redistributeModel` resolves status="ready" with num_nodes=2. | None. | 1. Render `<Inference />`. 2. Wait for the Redistribute button to enable. 3. Click it. | `redistributeModel` called with `'mamba-130m'`; `loadModel` still only called once (from the initial selection). |
| TC-INF-03 | FR-WUI-04 | Inference | Shows an error message when the model fails to load | Verify a dispatch failure during preload surfaces as a visible error instead of silently leaving the UI disabled. | `loadModel`/`getModelStatus` resolve status="error" with a "no available nodes" message. | error="no available nodes in the registry". | 1. Render `<Inference />`. | "Failed to load model: no available nodes..." message present in the document. |
...
| TC-TP-02 | FR-WUI-05 | Topology | Shows the real pipeline once a model has dispatched | Verify the graph reflects the actual per-node layer assignments and first/last roles returned by `GET /topology`, not mock data. | `listNodes` resolves one node; `getTopology` resolves one assignment (node-0, layers [0,24), is_first=is_last=True). | model_name="mamba-130m". | 1. Render `<Topology />`. | "Showing the active pipeline for \"mamba-130m\"." message shown; "layers [0, 24)", "first + last" role label, and "Orchestrator" all present. |
