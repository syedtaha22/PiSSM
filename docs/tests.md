...
| TC-HA-13 | FR-IE-05, FR-IE-06 | TestPostInfer | Success path returns output and latency | Verify a successful request returns generated text plus timing, per-node memory, and token-count fields. | ModelRegistry with mamba-130m registered; `plan_dispatch`, `ModelStore`, `PipelineRunner`, `AutoTokenizer.from_pretrained` mocked; one-node fake plan. | max_new_tokens=1. | 1. Call `POST /infer`. | 200; output="hello world"; node_latencies_ms=[5.0]; peak_memory_mb=[260]; num_nodes=1; num_tokens=1; latency_ms >= 0; runner.load called once. |
| TC-HA-14 | FR-IE-04 | TestPostInfer | Second call reuses resident runner | Verify a second request for the same model reuses the loaded pipeline instead of dispatching and loading shards again. | Same mocks as TC-HA-13. | Two requests, same model_name. | 1. Call `POST /infer` twice. | PipelineRunner constructed once; plan_dispatch called once; runner.load called once; runner.run_forward called twice. |
...
Tests for `MambaShardModule` forward pass shapes using lightweight mock sub-modules. No real model weights required.

| Test Case ID | Requirement | Test Suite | Title | Description | Pre-conditions | Test Data | Test Steps | Expected Result |
|---|---|---|---|---|---|---|---|---|
| TC-SM-01 | FR-IE-04 | TestMambaShardModuleForward | First shard forward shape | Verify the first shard embeds token IDs to hidden states. | None. | batch=1, seq=5, d_model=8, vocab=100, nn.Embedding + nn.Identity layer. | 1. Create first shard. 2. Pass token IDs. | output.shape == (1, 5, 8). |
| TC-SM-02 | FR-IE-04 | TestMambaShardModuleForward | Middle shard forward shape | Verify a middle shard passes hidden states through unchanged. | None. | batch=1, seq=5, d_model=8, nn.Identity layer. | 1. Create middle shard. 2. Pass hidden states. | output.shape == (1, 5, 8). |
| TC-SM-03 | FR-IE-04 | TestMambaShardModuleForward | Last shard forward shape | Verify the last shard projects hidden states to logits. | None. | batch=1, seq=5, d_model=8, vocab=100, nn.Identity + LayerNorm + Linear. | 1. Create last shard. 2. Pass hidden states. | output.shape == (1, 5, 100). |
